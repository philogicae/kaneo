    ...
    .where(
      and(
        ...
        reminderType === "configured_before"
          ? sql`${taskTable.dueDate} - (COALESCE(${userNotificationPreferenceTable.dueDateReminderLeadTimeMinutes}, 1440) * ${MINUTE_MS}) BETWEEN ${windowStart.getTime()} AND ${windowEnd.getTime()}`
