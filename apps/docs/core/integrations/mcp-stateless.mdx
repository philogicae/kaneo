...
The token may be an OAuth session token or an [API key](/api-reference/authentication),
so clients that cannot run the OAuth flow can use a static credential. Modern
requests use a fresh `McpServer` instance and hold no process-local
