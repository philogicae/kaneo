...
The Notifications page groups every notification surface into tabs:

- **General** — the account delivery channels and workspace rules described on this page
- **Telegram** — bots, chats, and routing rules for the unified Telegram configuration
- **Discord** — every project's Discord webhook, opened inline for editing
- **Slack** — every project's Slack incoming webhook, opened inline for editing
...
Delivery covers the notification types that also create an entry in the in-app inbox:
...
- `task_mention` — someone @mentions you in a task description or comment
- `due_date_reminder` — a task is due soon (the per-channel lead time)
- `task_overdue` — a task is past its due date
- `appointment_created` and `appointment_updated`

Mentions are a distinct event from comments: the channel integrations (Telegram, Discord, Slack) expose separate toggles for the two, so you can subscribe to mentions without receiving every comment.
